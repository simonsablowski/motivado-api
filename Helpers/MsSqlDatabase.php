...
class MsSqlDatabase extends OdbcDatabase {
	...
	public static function connect() {
		...
		if (!self::setLink(odbc_connect($connectionString, self::getConfiguration('user'), self::getConfiguration('password')))) {
			throw new Error('Cannot connect to database', array('driver' => self::getConfiguration('driver'), 'server' => self::getConfiguration('server'), 'name' => self::getConfiguration('name')));
		}
	}
}